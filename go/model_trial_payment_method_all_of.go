/*
 * Tenant API
 *
 * Tenant API
 *
 * API version: 0.0.1
 * Contact: abc@layer.fr
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package graalsystems/sdk

import (
	"encoding/json"
	"time"
)

// TrialPaymentMethodAllOf struct for TrialPaymentMethodAllOf
type TrialPaymentMethodAllOf struct {
	Type *string `json:"type,omitempty"`
	Expire *time.Time `json:"expire,omitempty"`
}

// NewTrialPaymentMethodAllOf instantiates a new TrialPaymentMethodAllOf object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewTrialPaymentMethodAllOf() *TrialPaymentMethodAllOf {
	this := TrialPaymentMethodAllOf{}
	var type_ string = "trial"
	this.Type = &type_
	return &this
}

// NewTrialPaymentMethodAllOfWithDefaults instantiates a new TrialPaymentMethodAllOf object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewTrialPaymentMethodAllOfWithDefaults() *TrialPaymentMethodAllOf {
	this := TrialPaymentMethodAllOf{}
	var type_ string = "trial"
	this.Type = &type_
	return &this
}

// GetType returns the Type field value if set, zero value otherwise.
func (o *TrialPaymentMethodAllOf) GetType() string {
	if o == nil || o.Type == nil {
		var ret string
		return ret
	}
	return *o.Type
}

// GetTypeOk returns a tuple with the Type field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TrialPaymentMethodAllOf) GetTypeOk() (*string, bool) {
	if o == nil || o.Type == nil {
		return nil, false
	}
	return o.Type, true
}

// HasType returns a boolean if a field has been set.
func (o *TrialPaymentMethodAllOf) HasType() bool {
	if o != nil && o.Type != nil {
		return true
	}

	return false
}

// SetType gets a reference to the given string and assigns it to the Type field.
func (o *TrialPaymentMethodAllOf) SetType(v string) {
	o.Type = &v
}

// GetExpire returns the Expire field value if set, zero value otherwise.
func (o *TrialPaymentMethodAllOf) GetExpire() time.Time {
	if o == nil || o.Expire == nil {
		var ret time.Time
		return ret
	}
	return *o.Expire
}

// GetExpireOk returns a tuple with the Expire field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TrialPaymentMethodAllOf) GetExpireOk() (*time.Time, bool) {
	if o == nil || o.Expire == nil {
		return nil, false
	}
	return o.Expire, true
}

// HasExpire returns a boolean if a field has been set.
func (o *TrialPaymentMethodAllOf) HasExpire() bool {
	if o != nil && o.Expire != nil {
		return true
	}

	return false
}

// SetExpire gets a reference to the given time.Time and assigns it to the Expire field.
func (o *TrialPaymentMethodAllOf) SetExpire(v time.Time) {
	o.Expire = &v
}

func (o TrialPaymentMethodAllOf) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Type != nil {
		toSerialize["type"] = o.Type
	}
	if o.Expire != nil {
		toSerialize["expire"] = o.Expire
	}
	return json.Marshal(toSerialize)
}

type NullableTrialPaymentMethodAllOf struct {
	value *TrialPaymentMethodAllOf
	isSet bool
}

func (v NullableTrialPaymentMethodAllOf) Get() *TrialPaymentMethodAllOf {
	return v.value
}

func (v *NullableTrialPaymentMethodAllOf) Set(val *TrialPaymentMethodAllOf) {
	v.value = val
	v.isSet = true
}

func (v NullableTrialPaymentMethodAllOf) IsSet() bool {
	return v.isSet
}

func (v *NullableTrialPaymentMethodAllOf) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableTrialPaymentMethodAllOf(val *TrialPaymentMethodAllOf) *NullableTrialPaymentMethodAllOf {
	return &NullableTrialPaymentMethodAllOf{value: val, isSet: true}
}

func (v NullableTrialPaymentMethodAllOf) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableTrialPaymentMethodAllOf) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


