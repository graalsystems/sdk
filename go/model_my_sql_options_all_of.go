/*
Tenant API

Tenant API

API version: 0.0.1
Contact: abc@layer.fr
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package sdk

import (
	"encoding/json"
)

// MySQLOptionsAllOf struct for MySQLOptionsAllOf
type MySQLOptionsAllOf struct {
	Type *string `json:"type,omitempty"`
}

// NewMySQLOptionsAllOf instantiates a new MySQLOptionsAllOf object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewMySQLOptionsAllOf() *MySQLOptionsAllOf {
	this := MySQLOptionsAllOf{}
	var type_ string = "mysql"
	this.Type = &type_
	return &this
}

// NewMySQLOptionsAllOfWithDefaults instantiates a new MySQLOptionsAllOf object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewMySQLOptionsAllOfWithDefaults() *MySQLOptionsAllOf {
	this := MySQLOptionsAllOf{}
	var type_ string = "mysql"
	this.Type = &type_
	return &this
}

// GetType returns the Type field value if set, zero value otherwise.
func (o *MySQLOptionsAllOf) GetType() string {
	if o == nil || o.Type == nil {
		var ret string
		return ret
	}
	return *o.Type
}

// GetTypeOk returns a tuple with the Type field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MySQLOptionsAllOf) GetTypeOk() (*string, bool) {
	if o == nil || o.Type == nil {
		return nil, false
	}
	return o.Type, true
}

// HasType returns a boolean if a field has been set.
func (o *MySQLOptionsAllOf) HasType() bool {
	if o != nil && o.Type != nil {
		return true
	}

	return false
}

// SetType gets a reference to the given string and assigns it to the Type field.
func (o *MySQLOptionsAllOf) SetType(v string) {
	o.Type = &v
}

func (o MySQLOptionsAllOf) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Type != nil {
		toSerialize["type"] = o.Type
	}
	return json.Marshal(toSerialize)
}

type NullableMySQLOptionsAllOf struct {
	value *MySQLOptionsAllOf
	isSet bool
}

func (v NullableMySQLOptionsAllOf) Get() *MySQLOptionsAllOf {
	return v.value
}

func (v *NullableMySQLOptionsAllOf) Set(val *MySQLOptionsAllOf) {
	v.value = val
	v.isSet = true
}

func (v NullableMySQLOptionsAllOf) IsSet() bool {
	return v.isSet
}

func (v *NullableMySQLOptionsAllOf) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableMySQLOptionsAllOf(val *MySQLOptionsAllOf) *NullableMySQLOptionsAllOf {
	return &NullableMySQLOptionsAllOf{value: val, isSet: true}
}

func (v NullableMySQLOptionsAllOf) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableMySQLOptionsAllOf) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


